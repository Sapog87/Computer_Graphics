...
namespace lab8
{
    public class Graphics3D
    {
        ...
        public Graphics3D(Graphics graphics, Matrix transformation, double width, double height, Point cameraPos)
        {
        ...
        private Point SpaceToNormalized(Point v)
        {
        ...
        {
            return new Point(v.X / v.W, v.Y / v.W, v.Z / v.W);
        }

        public Point NormalizedToScreen(Point v)
        {
            return new Point((v.X + 1) / 2 * Width, (-v.Y + 1) / 2 * Height, v.Z);
        }

        private Point SpaceToScreenCoordinate(Point spaceVertex)
        {
        ...
        private bool ShouldBeDrawn(Point vertex)
        {
            return ((vertex.X >= 0 && vertex.X < Width) && (vertex.Y >= 0 && vertex.Y < Height) && (vertex.Z < 1) && (vertex.Z > -1));
        }
        ...
        public void DrawPoint(Point a, Color color)
        {
            ...
            if (ShouldBeDrawn(A)) ColorBuffer.SetPixel((int)Math.Ceiling(A.X), (int)Math.Ceiling(A.Y), color);
        }

        public void DrawLine(Point a, Point b, Pen pen)
        {
            var t = SpaceToNormalized(a);
            if (t.Z < -1 || t.Z > 1) return;
            var A = NormalizedToScreen(t);
        ...
        private Point Interpolate(Point a, Point b, double f)
        {
            return new Point(Interpolate(a.X, b.X, f), Interpolate(a.Y, b.Y, f), Interpolate(a.Z, b.Z, f));
        }
        ...
        public void DrawPolygon(Vertex a, Vertex b, Vertex c)
        {
            a = SpaceToScreen(a);
            b = SpaceToScreen(b);
            c = SpaceToScreen(c);
